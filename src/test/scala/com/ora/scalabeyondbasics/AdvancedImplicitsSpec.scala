...
//      pending
      ...
      implicit class ExclaimWrapper[T](x: T) {
        def exclaim: String = s"${x.toString}!"
      }

      10.exclaim should be ("10!")
      "Hello".exclaim should be ("Hello!")
      List(1, 2, 3).exclaim should be ("List(1, 2, 3)!")
      1.1.exclaim should be ("1.1!")
    }
  ...
  describe("Locating implicits recipes") {
    ...
    it("""is used in a real world scenario like BigInt""".stripMargin) {

      val bi = BigInt("32492349439")

      (bi + 40) should be (BigInt("32492349479"))
    }
    ...
    it(
      """Lab: Use scala.collection.JavaConverters to convert the Java call,
        ...
        |  a Scala collection. Then do something fun with it like find all the
        |  time zones in Asia and sort them.  Everything can be
        |  done inside of this test. Consider REPL as a handy tool.""".stripMargin) {

//      pending

      def getAllTimeZonesFromContinent(s: String): Seq[String] = {
  ...
  describe(
    ...
    it("""uses the operator, <:< which will test if A is a subtype of B, see List toMap API docs as an example""") {
      pending
    }
  }
  ...
  describe(
    ...
    it(
      ...
      object MyPredef {
        implicit val orderTeamsByCity: Ordering[Team] = (x: Team, y: Team) => x.city compare y.city

        implicit val orderTeamsByMascot: Ordering[Team] = (x: Team, y: Team) => x.mascot compare y.mascot
      }

      import MyPredef.orderTeamsByCity

      val teams: List[Team] = List(Team("Cincinnati", "Bengals"),
      ...
      teams.sorted.min should be (Team("Arizona", "Diamondbacks"))
    }
  }
}
